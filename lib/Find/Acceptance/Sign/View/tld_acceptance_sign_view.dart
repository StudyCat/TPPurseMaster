...
class _TLDAcceptanceSignViewState extends State<TLDAcceptanceSignView> {
    ...
    return CalendarViewWidget(
      ...
      dayWidgetBuilder: (DateModel model) {
        ...
        if (widget.userInfoModel != null) {
          if (model.isBefore(_todayDateModel)) {
            if (_isSignDay(model, _todayDateModel)){
              ...
              type = TLDAcceptanceSignDayViewType.signFail;
            }
        ...
        return GestureDetector(
          ...
          child: TLDAcceptanceSignDayView(type: type, day: model.day),
        );
      },
    );
  }
  ...
  bool _isSignDay(DateModel model, DateModel todayDateModel) {
    List dayList = [];
    for (TLDSignModel item in widget.userInfoModel.signList) {
