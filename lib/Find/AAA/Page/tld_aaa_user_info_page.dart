...
class TLDAAAUserInfoPage extends StatefulWidget {
  TLDAAAUserInfoPage({Key key,this.userId}) : super(key: key);
...
class _TLDAAAUserInfoPageState extends State<TLDAAAUserInfoPage> {
  ...
  TLDAAAUserInfoModelManager _modelManager;
  ...
  void initState() {
    ...
    _modelManager = TLDAAAUserInfoModelManager();
    _getUserInfo();
  }
  ...
  void _getUserInfo(){
    ...
    _modelManager.getUserInfo(widget.userId, (TLDAAAUserInfo userInfo){
      if (mounted){
        setState(() {
          _isLoading = false;
          _userInfo = userInfo;
    ...
    return _userInfo != null ? ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        if (index == 0){
          ...
          return Padding(
            padding:  EdgeInsets.only(top:ScreenUtil().setHeight(2),left: ScreenUtil().setWidth(30),right: ScreenUtil().setWidth(30)),
            child: TLDClipCommonCell(type: TLDClipCommonCellType.normal,title: title,content:content,titleStyle:  TextStyle(
