...
class TLDPublishMissionPage extends StatefulWidget {
  TLDPublishMissionPage({Key key,this.walletAddress}) : super(key: key);

  final String walletAddress;

  @override
  _TLDPublishMissionPageState createState() => _TLDPublishMissionPageState();
}

class _TLDPublishMissionPageState extends State<TLDPublishMissionPage> {
  List titleList = ['钱包', '钱包余额', '发布总量', '任务等级', '其他费用扣除比例（任务奖励金）', '其他费用预计扣除（任务奖励金）', '预计到账', '收款方式'];

  TLDPublishMissionFormModel _formModel;

  TLDPublishMissionModelManager _modelManager;
  ...
  void initState() {
    ...
    _formModel = TLDPublishMissionFormModel();
    _formModel.walletAddress = widget.walletAddress;
    _formModel.totalCount = '0.0';

    _modelManager = TLDPublishMissionModelManager();
  ...
  void _getWalletInfo(){
    _modelManager.getWalletInfo(widget.walletAddress, (TLDWalletInfoModel infoModel){
      setState(() {
        _isLoading = false;
        ...
        if (index == 0) {
          ...
          return TLDExchangeNormalCell(
            ...
            didClickCallBack: (){
              ...
              Navigator.push(context, MaterialPageRoute(builder: (context)=> TLDNewMissionChooseMissionLevelPage(didChooseLevelCallBack: (TLDMissionLevelModel levelModel){
                setState(() {
